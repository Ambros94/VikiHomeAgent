...
Le interfacce in linguaggio naturale permettono all’utente di controllare un sistema attraverso la propria voce. Esse sono estremamente efficaci, ma vengono spesso criticate per la loro rigidità.\\
I metodi generalmente utilizzati nella costruzione di queste interfacce sono basati sull’utilizzo di un sistema di regole, dette grammatiche, al quale l’utente si deve attenere.\\
Il sistema proposto propone un metodologia di associazione delle frasi pronunciate dall’utente ai comandi disponibili della casa senza l’utilizzo di nessuna regola predefinita; essa è basata sul confronto di vettori che rappresentano le parole.\\
L’assenza di regole e la possibilità di insegnare comandi al sistema garantiscono una maggior flessibilità del sistema, rendendo la conversazione con l’Agente più simile a una conversazione reale.\\
Il sistema si è dimostrato in grado di comprendere il 72\% delle frasi, scelti tra 31 comandi disponibili nel sistema. Migliori performance possono essere raggiunte insegnando le espressioni gergali al sistema.\\
Il sistema è stato realizzato per la lingua inglese, ma è predisposto per l’aggiunta di lingue.
