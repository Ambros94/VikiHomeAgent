...
\end{itemize}
\section{Vantaggi}
Si è scelto di separare il sistema in diverse componenti per favorirne la riutilizzabilità. Si è cercato di ridurre al minimo le interdipendenze, cercando di rendere ogni componente autonomo e utilizzabile in altri contesti.
...
Inoltre la trasmissione di informazione su un canale è più onerosa e lenta rispetto alla condivisione dello stesso spazio di memoria, ma per la quantità di informazione che è necessario trasmettere questa differenza non è stata ritenuta significativa.
...
Il cervello deve essere informato dal braccio a proposito di quali sono i comandi che possono essere eseguiti, quest'operazione avviene una sola volta, al boot del sistema. 

L'informazione viene reperita, seguendo l'architettura REST, attraverso una chiamata all'indirizzo GET \textit{"/metadata"}. Essa ritorna una struttura dati JSON che informa il sistema, con un formalismo documentato nel capitolo apposito.
...
Questo tipo di trasmissione crea un canale, che viene mantenuto sempre aperto e che è sempre pronto per comunicare diti in modo bi-direzionale.
