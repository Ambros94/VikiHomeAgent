...
\setcounter{page}{1}
\problemchapter{Abstract italiano}
Le interfacce basate su riconoscimento vocale sono estremamente efficaci, ma vengono spesso criticate per la loro rigidità.\\
I metodi generalmente utilizzati per la costruzione di queste interfacce sono basati su un sistema di regole, dette grammatiche, alle quali l’utente si deve attenere per l’interazione.\\
Il sistema introduce una nuova metodologia di associazione delle frasi pronunciate dall’utente ai comandi disponibili nella casa, cioè attraverso l'astrazione delle parole in vettori e alla comparazione di essi, senza l'ausilio di un sistema di regole.\\
Il sistema è sempre in ascolto e viene attivato alla pronuncia del suo nome proprio, inoltre è predisposto per l’apprendimento di nuovi comandi, insegnati vocalmente dall’utente.\\
...
\problemchapter{Abstract inglese}
Interfaces based on speech recognition are extremely effective, but are often criticized for their rigidity. \\
Those interfaces are usually built on rules systems, called grammars,that the user must follow during the interaction. \\
The system introduces a new method to combine sentences pronounced by the user to commands available in the house, that is done through the abstraction of words into vectors and comparing them, without the aid of a system of rules. \\
The system is always listening and is activated at the pronunciation of his own name, it is also designed for learning new user-taught commands vocally. \\
The absence of grammars and the chance of teaching ensures a greater flexibility, making the conversation with the Agent more similar to an actual conversation. \\
...
\begin{figure}[H]
\centering
\includegraphics[angle=90,scale=0.4]{gantt}
\caption{GANTT}
\label{fig:pose}
\end{figure}
\chapter{Conclusione}
